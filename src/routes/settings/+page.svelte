<script lang="ts">
  ...
  import { lfmBeginAuth, lfmCompleteAuth, lfmDisconnect } from '$lib/api';
  ...
  const initialLfm = untrack(() => data.lastFm);

  // ── Form state ────────────────────────────────────────────────────────────
  let lastFmApiKey = $state(initial.lastFmApiKey);
  let lastFmSharedSecret = $state('');
  let lastFmSharedSecretConfigured = $state(initial.lastFmSharedSecretConfigured);
  let showSharedSecret = $state(false);
  let recommendationProvider = $state(initial.recommendationProvider);
  let metadataProvider = $state(initial.metadataProvider);
  let showApiKey = $state(false);

  // ── Last.fm account state ─────────────────────────────────────────────────
  let lfmConnected = $state(initialLfm.connected);
  let lfmUsername = $state(initialLfm.username);
  ...
  // ── Profiles state ────────────────────────────────────────────────────────
  let profiles = $state<Profile[]>(untrack(() => data.profiles));
  ...
  // ── Connection status ─────────────────────────────────────────────────────
  type ServiceStatus = 'checking' | 'online' | 'offline' | 'missing';
  ...
  async function save() {
    ...
    try {
      ...
      // Only send shared secret if the user has typed a new value
      if (lastFmSharedSecret.trim()) {
        body.LASTFM_SHARED_SECRET = lastFmSharedSecret.trim();
      }
  ...
  async function saveProfile() {
    ...
    try {
      ...
      const res = isNew
        ...
        : await fetch(`/api/profiles/${profileDraft.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
      ...
      if (isNew) {
        ...
        profiles = profiles.map(p => p.id === payload.profile.id ? payload.profile : p);
      }
      dialogOpen = false;
