<script lang="ts">
  ...
  async function load() {
    ...
    try {
      ...
      const artistSeed = profile.topSongs.find((s) => norm(s.artist) === norm(artist));
      ...
      if (artistSeed) {
        ...
        result = hits.filter((s) => norm(s.artist) === norm(artist)).slice(0, 20);
      }
  ...
  function playAll() {
    ...
    playingFrom.set({ type: 'playlist', name: `${artist} Mix`, href: HREF });
    playQueue(songs, 0);
  }
